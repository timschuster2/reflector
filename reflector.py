"""
Reflector v1.1 — Timothy's AI Agent Swarm
Runs daily at 6:30am AEST (7:30pm UTC) via Railway cron.

Responsibilities:
1. Check + set run lock in system_config (prevents double-run)
2. Read swarm_events, swarm_tasks, swarm_reflections from Supabase
3. Rebuild MEMORY.md (200-line cap, last 7 days activity)
...
# ── Build MEMORY.md (200-line cap) ─────────────────────────────────────────────
def build_memory_md(events: list, tasks: list, reflections: list) -> str:
    now = datetime.now(timezone.utc).strftime("%d %b %Y %H:%M UTC")
    lines = [
        "# MEMORY.md — Timothy's AI Agent Swarm",
    ...
    lines += [
        ...
        "- timschuster.dev → Vercel (timschuster-deploy)",
        "- Ideation scanner → Railway cron 8am AEST",
        "- Swarm Telegram bot → Railway",
        "- Supabase swarm-data (isolated from trading-data)",
        "- Trading swarm v4.0 → trading-swarm repo",
        "",
        "## Key Rules",
        "- STP: Projects → outputs file → local save → Claude Code deploys",
        "- Never cross-write swarm-data ↔ trading-data",
...
# ── Build per-agent MEMORY.md (50-line cap) ────────────────────────────────────
def build_agent_memory_md(agent_name: str, events: list, tasks: list) -> str:
    ...
    lines = [
        f"# {agent_name.upper()} — Agent Memory",
        f"# Generated by Reflector v1.1 at {now}. Never hand-edit.",
        "",
        "## My Recent Activity",
    ]
...
# ── Main ───────────────────────────────────────────────────────────────────────
def main():
    print(f"Reflector starting — {datetime.now(timezone.utc).isoformat()}")

    if not acquire_lock():
        print("Lock already held — skipping run")
        telegram("⚠️ Skipped — already running", urgent=False)
        return
    ...
    try:
        ...
        memory_md = build_memory_md(events, tasks, reflections)
        ...
        summary = f"✅ Reflector ran. Events: {len(events)}, Tasks: {len(tasks)}, Agent files: {agent_count}"
        print(summary)
        telegram(summary)
        ...
        print(f"ERROR: {e}\n{tb}")
        telegram(f"🔴 FAILED: {str(e)[:200]}", urgent=True)
